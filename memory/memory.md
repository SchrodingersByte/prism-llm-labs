...
## Deploy wiring — Vercel env + SDK publish (DONE 2026-06-21)
- **Vercel env:** linked all 26 real app vars from `apps/web/.env.local` to Production+Preview+Development (78 entries) on `dip-dey-s-projects/prism-llm-labs` (authed `schrodingersbyte`; project is linked under `apps/web/.vercel`, not repo root). Excluded the ~21 `VERCEL_*`/`TURBO_*`/`NX_DAEMON` runtime vars. **CLI gotchas:** run with `--cwd apps/web`; `preview` needs an explicit empty git-branch positional non-interactively → `vercel env add NAME preview "" --value V --yes` (plain `--yes` alone still errors `git_branch_required`); inside a `while read < file` loop the inner `vercel` must get `</dev/null` or it consumes the loop's stdin. **Still TODO (user):** `ENCRYPTION_SECRET` is NOT in `.env.local` — must be generated + set across the 3 envs (provider-key AES). Auto-generating it was correctly blocked.
- **SDK publish:** re-enabled `publish-npm.yml` + `publish-pypi.yml` (were `.disabled`); switched the pypi workflow from OIDC trusted-publishing to `PYPI_API_TOKEN`. Set `NPM_TOKEN` + `PYPI_API_TOKEN` repo secrets. Tagged + published green: **`@prism-llm-labs/sdk` 0.5.0** (npm; was 0.4.4) + **`prism-llm-labs` 0.4.0** (PyPI; was 0.3.4). Other 5 npm pkgs already at published versions (nothing new). **SECURITY:** both tokens were pasted in plaintext chat → user must rotate them + re-set the GH secrets. **Source-of-truth:** packages were historically published from the dev `prism` repo; staging now publishes too — pick one owner to avoid duplicate-version rejections.
