/**
 * GET /api/metrics/customers/[customerId]/daily
 * Daily spend timeseries for one customer. Manager-only.
 */
...
import { getCustomerDailyTimeseries } from "@/lib/tinybird/queries";
...
export async function GET(req: NextRequest, { params }: { params: { customerId: string } }) {
  ...
  const data = await getCustomerDailyTimeseries(member.org_id, params.customerId, from, to);
