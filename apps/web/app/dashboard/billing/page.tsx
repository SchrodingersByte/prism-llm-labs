...
function UsagePanel({ usage, retentionDays }: { usage: BillingStatus["usage"]; retentionDays: number }) {
  ...
  const tone = usage.pctUsed >= 100 ? "bg-rose-500" : usage.pctUsed >= 80 ? "bg-amber-500" : "bg-primary";
  return (
    <div className="mt-3 space-y-2">
      ...
      {usage.overage > 0 && <p className="text-xs text-rose-500">{compact.format(usage.overage)} events over quota</p>}
...
export default function BillingPage() {
  ...
  return (
    <div>
      <PageHeader title="Billing" description="Plan, usage, and payment region for this workspace." />
            ...
            <div className="flex items-center gap-1.5">
              {(["US", "IN"] as const).map((r) => (
                <Button
                  key={r}
                  size="sm"
                  variant={status?.billing.region === r ? "default" : "outline"}
                  disabled={!isOwner || busy === "region"}
                  onClick={() => setRegion(r)}
                >
