...
interface LogRow {
  id: string; model: string; provider: string; input_tokens: number; output_tokens: number;
  cost_usd: number; latency_ms: number | null; status_code: number | null;
  session_id: string | null; created_at: string; trace_id: string | null;
...
/** Request-log explorer. Pass `projectId` to scope to one project (project tier). */
export function LogsExplorer({ projectId }: { projectId?: string }) {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["logs", projectId ?? "", status, query, offset],
    queryFn: ({ signal }) => apiGet<{ data: LogRow[]; total: number }>("/api/logs", {
          ...
          <div className="flex items-center gap-0.5 rounded-md border border-border p-0.5">
            {STATUSES.map(([v, label]) => (
              <button key={v} onClick={() => setStatusReset(v)}
                className={cn("rounded px-2 py-1 text-xs transition-colors", status === v ? "bg-secondary text-foreground" : "text-muted-foreground hover:text-foreground")}>{label}</button>
            ))}
          </div>
