"""
Pricing table — price per 1,000,000 tokens (USD).
...
Prefix-matching in _resolve_model() handles versioned IDs automatically:
  "gpt-4o-2024-11-20" → "gpt-4o" pricing.
Unknown models return None → cost recorded as $0.
...
MODEL_PRICING: dict[str, dict] = {
    ...
    # Reasoning tokens billed at output price.
...
def _resolve_model(model: str) -> Optional[dict]:
    """Exact match first; then longest-prefix match for versioned model IDs."""
...
def normalize_model_name(model: str) -> str:
    """Return the canonical pricing-table key for a versioned model name.

    Examples:
        "gpt-4o-mini-2024-07-18" → "gpt-4o-mini"
        "gpt-4o-mini"            → "gpt-4o-mini"
        "my-custom-deployment"   → "my-custom-deployment"
    """
    if model in MODEL_PRICING:
        return model
    for key in _SORTED_KEYS:
...
def calculate_cost(
    ...
    return (
        ...
        + (cached_tokens * p.get("cached_input", p["input"])) / 1_000_000
